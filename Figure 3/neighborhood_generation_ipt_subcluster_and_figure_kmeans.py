# -*- coding: utf-8 -*-
"""Neighborhood generation iPT Subcluster and figure kmeans.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1X5FlVDWBTjVFQY_ZoxLxsqur0rR8A_ew
"""
...
new_adata.obs['in_both']=adata.obs['in_both'].copy()

new_adata_subset=new_adata[new_adata.obs['total_neighbors_200']>50]
new_adata_subset

new_adata_subset=new_adata_subset[new_adata_subset.obs['annotation_post_scanvi70_broad']=='iPT']
new_adata_subset
...
cell_identities = {0: 'iPT ME 1', 1: 'iPT ME 2', 2: 'iPT ME 3', 3: 'iPT ME 4'}
new_adata_subset.obs["cluster_labels_annotated_iPT"] = new_adata_subset.obs['cluster_labels'].map(cell_identities).astype('category')
...
for group in de_results['names'].dtype.names:
        ...
        df = df[~df['Gene'].str.endswith('_40')]
        df = df[~df['Gene'].str.endswith('_20')]
        print(df.head(40))

markers = [ "PT_20","TAL_20","Podo_20", "Immune_20","Fibroblast_20","iPT_20","iTAL_20", "CNT_20", ]

sc.pl.dotplot(new_adata_subset, markers, groupby='iPT_subcluster_ME',cmap='Blues', log = False)
...
# Create the dot plot
dotplot = sc.pl.dotplot(new_adata_subset, markers, groupby='iPT_subcluster_ME',
...
plt.tight_layout()
plt.savefig('iPT MEs 20um.png', dpi=450, bbox_inches='tight')
# Show the plot
plt.show()
...
# Create the dot plot
dotplot = sc.pl.dotplot(new_adata_subset, markers, groupby='iPT_subcluster_ME',
                        cmap='Blues', log=False, show=False, swap_axes=False, dot_max=1, vmax=0.45)
...
# Create the dot plot
dotplot = sc.pl.dotplot(new_adata_subset, markers, groupby='iTAL_subcluster_ME',
                        cmap='Blues', log=False, show=False, swap_axes=False, dot_max=1, vmax=0.45)
...
# Define markers
markers = ["PT_40", "TAL_40", "Podo_40", "Immune_40", "Fibroblast_40", "iPT_40", "iTAL_40", "CNT_40"]

# Create the dot plot
dotplot = sc.pl.dotplot(new_adata_subset, markers, groupby='cluster_labels_annotated_iPT',
                        cmap='Blues', log=False, show=False, swap_axes=True, dot_max=1, vmax=0.45)
...
# Adjust the plot layout
plt.tight_layout()
plt.savefig('iPT MEs 40um.png', dpi=450, bbox_inches='tight')
# Show the plot
plt.show()
...
# Define markers
markers = ["PT_60", "TAL_60", "Podo_60", "Immune_60", "Fibroblast_60", "iPT_60", "iTAL_60", "CNT_60"]
...
# Adjust the plot layout
plt.tight_layout()
plt.savefig('iPT MEs 60um.png', dpi=450, bbox_inches='tight')
# Show the plot
plt.show()

dotplot

markers = ["PT_40","TAL_40","Podo_40","IC A_40", "IC B_40", "Immune_40","Fibroblast_40","iPT_40", "CNT_40","iTAL_40"]

sc.pl.dotplot(new_adata_subset, markers, groupby='cluster_labels_annotated_iPT',cmap='Blues', log = False)

markers = ["PT_60","TAL_60","Podo_60","IC A_60", "IC B_60", "Immune_60","Fibroblast_60","iPT_60", "CNT_60","iTAL_60", "VSMC_60"]
...
data = pd.DataFrame({'sample': samples, 'cluster_labels_annotated_iPT': cluster_labels})

# Group by 'sample' and 'cluster_labels_annotated' and count the occurrences
grouped_data = data.groupby(['sample', 'cluster_labels_annotated_iPT']).size().unstack(fill_value=0)
desired_order = [
...
plt.tight_layout()
plt.savefig('iPT total ME abundance.png', dpi=450, bbox_inches='tight')
plt.show()
...
"""lets make piecharts for 20µm environment"""
...
filtered_df['cluster_labels']=new_adata_subset.obs['in_both'].copy()

filtered_df['cluster_labels']

filtered_df=filtered_df[filtered_df['cluster_labels']=='yes']
...
filtered_df['cluster_labels'] = filtered_df['cluster_labels'].cat.remove_unused_categories()

normalized_df.columns
normalized_df['other']=normalized_df['TAL']+normalized_df['PT']+normalized_df['IC B']+normalized_df['PC']+normalized_df['DTL_ATL']+normalized_df['EC_Peritub']+normalized_df['EC_DVR']+normalized_df['VSMC']+normalized_df['CNT']+normalized_df['Podo']++normalized_df['EC_glom']+normalized_df['PEC']+normalized_df['IC A']+normalized_df['MC1']

normalized_df=normalized_df.drop(['TAL', 'PT',  'IC B', 'PC', 'DTL_ATL',  'EC_Peritub',
       'EC_DVR', 'VSMC', 'DCT', 'CNT', 'Podo', 'EC_glom',
       'PEC', 'IC A', 'MC1'], axis=1)
...
for label in normalized_df.index:
    ...
    # Save the figure
    plt.savefig(f'in_both_Cluster_20um_pie_chart_simplified_{label}.png', dpi=450, bbox_inches='tight')
    plt.show()
...
# Calculate 'other' and drop the specified columns
normalized_df['other'] = normalized_df[['TAL', 'PT', 'IC B', 'PC', 'DTL_ATL', 'EC_Peritub',
                                        'EC_DVR', 'VSMC', 'CNT', 'Podo', 'EC_glom',
                                        'PEC', 'IC A', 'MC1']].sum(axis=1)
normalized_df = normalized_df.drop(['TAL', 'PT', 'IC B', 'PC', 'DTL_ATL', 'EC_Peritub',
                                    'EC_DVR', 'VSMC', 'DCT', 'CNT', 'Podo', 'EC_glom',
                                    'PEC', 'IC A', 'MC1'], axis=1)
...
# Step 5: Plot horizontal stacked bar plot for each cluster label
for label in normalized_df.index:
    print(label)
    ...
    bottom = None
    for col, color in zip(normalized_df.columns, colors):
        plt.barh(label, normalized_df.loc[label, col], left=bottom, color=color, edgecolor='none')
        if bottom is None:
            bottom = normalized_df.loc[label, col]
        else:
            bottom += normalized_df.loc[label, col]
    ...
    plt.gca().set_xticks([])
    # Save the figure
    plt.savefig(f'in_both_Cluster_20um_horizontal_stacked_bar_{label}.png', dpi=450, bbox_inches='tight')
    plt.show()
...
for label in normalized_df.index:
    ...
    bottom = None
    for col, color in zip(normalized_df.columns, colors):
        plt.barh(label, normalized_df.loc[label, col], left=bottom, color=color, edgecolor='none', label=col)
        if bottom is None:
            bottom = normalized_df.loc[label, col]
        else:
            bottom += normalized_df.loc[label, col]
    ...
    # Remove the y-axis labeling and the y-axis and x-axis ticks
    plt.gca().set_yticklabels([])
    plt.gca().set_yticks([])
    plt.gca().set_xticks([])
...
# Extract the relevant columns
samples = new_adata_subset.obs['sample']
cluster_labels = new_adata_subset.obs['iPT_subcluster_ME']

# Create a DataFrame from the relevant columns
data = pd.DataFrame({'sample': samples, 'iPT_subcluster_ME': cluster_labels})

# Group by 'sample' and 'cluster_labels_annotated' and count the occurrences
grouped_data = data.groupby(['sample', 'iPT_subcluster_ME']).size().unstack(fill_value=0)
desired_order = [
    "Pediatric1", "HK2753", "HK3039", "HK3106", "HK3531", "HK3531_2",
    "HK3063", "HK3542", "HK2874", "HK2695", "HK3035", "HK3035_2",
     "HK2841", "HK2873","HK2844", "HK2844_2"
...
ax = normalized_count_df.plot(kind='bar', stacked=True, figsize=(6, 4), color=colors)
...
ax.spines['right'].set_visible(False)
plt.tight_layout()
plt.savefig('iPT relative niche amount less less less longer.png', dpi=450, bbox_inches='tight')
plt.show()
...
adata.obs['iPT_subcluster_ME']=new_adata_subset.obs['cluster_labels_annotated_iPT'].copy()
adata_all.obs['iPT_subcluster_ME']=new_adata_subset.obs['cluster_labels_annotated_iPT'].copy()

adata.obs['iPT_subcluster_ME'] = adata.obs['iPT_subcluster_ME'].cat.add_categories(['outlier'])
adata.obs["iPT_subcluster_ME"]=adata.obs["iPT_subcluster_ME"].fillna('outlier')

adata_all.obs['iPT_subcluster_ME'] = adata_all.obs['iPT_subcluster_ME'].cat.add_categories(['outlier'])
adata_all.obs["iPT_subcluster_ME"]=adata_all.obs["iPT_subcluster_ME"].fillna('outlier')

print(adata_all.obs['iPT_subcluster_ME'])

print(adata.obs['iPT_subcluster_ME'])
...
iPT_subset=adata_all[adata_all.obs['iPT_subcluster_ME']!='outlier']

iPT_subset.X = iPT_subset.layers["counts"]
sc.pp.normalize_total(iPT_subset, inplace=True)
sc.pp.log1p(iPT_subset)
sc.tl.rank_genes_groups(iPT_subset, groupby='iPT_subcluster_ME', method='wilcoxon', pts = True)
sc.pl.rank_genes_groups(iPT_subset, n_genes=25, sharey=False)

de_results = iPT_subset.uns['rank_genes_groups']
for group in de_results['names'].dtype.names:
        gene_names = de_results['names'][group]
        gene_scores = de_results['scores'][group]
        gene_logfoldchanges = de_results['logfoldchanges'][group]
...
markers = ["AZGP1","CUBN","CXCL14", "TNFRSF14","SLC13A3","GPX3", "ACSM2B","MAF",  "LRP2","IFITM3", "IL32",
           "VCAM1",
          "KRT7",'KRT19','SOX9', 'TNFRSF12A',"CRYAB","SPP1",'HAVCR1',"TPM1","MMP7",
            'SOX4',
           ]

# Ensure markers list only includes valid gene names
markers = [gene for gene in markers if gene in iPT_subset.var_names]

# Subset the .X matrix for the markers and convert to a DataFrame
gene_expression_df_unimputed = pd.DataFrame(
    iPT_subset[:, markers].X.toarray() if issparse(iPT_subset.X) else iPT_subset[:, markers].X,
    index=iPT_subset.obs_names,
    columns=markers
)

gene_expression_df_unimputed['iPT_subcluster_ME'] = iPT_subset.obs['iPT_subcluster_ME'].values
mean_expression_per_cluster_unimputed = gene_expression_df_unimputed.groupby('iPT_subcluster_ME').mean()
...
# Save the figure
plt.savefig('heatmap_iPT_ME.png', dpi=450, bbox_inches='tight')

# Show the plot
plt.show()

df = pd.DataFrame({
    'cluster_labels': iPT_subset.obs['iPT_subcluster_ME'],
    'type': iPT_subset.obs['type'],
    'sample':iPT_subset.obs['sample']
}, index=iPT_subset.obs_names)
df

df.to_csv('iPT_ME.csv', index=True)
...
# Save the plot
plt.tight_layout()
plt.savefig('relative_increase_iPT_heatmap.png', dpi=450)
plt.show()
...
iPT1 = pd.read_csv("/content/drive/MyDrive/Bernhard/Cosmx/After_Cleaning/iPT_subclustering/Microenvironments_iPT/iPT_iPT ME 1_DEG.csv")
iPT2 = pd.read_csv("/content/drive/MyDrive/Bernhard/Cosmx/After_Cleaning/iPT_subclustering/Microenvironments_iPT/iPT_iPT ME 2_DEG.csv")
iPT3 = pd.read_csv("/content/drive/MyDrive/Bernhard/Cosmx/After_Cleaning/iPT_subclustering/Microenvironments_iPT/iPT_iPT ME 3_DEG.csv")
iPT4 = pd.read_csv("/content/drive/MyDrive/Bernhard/Cosmx/After_Cleaning/iPT_subclustering/Microenvironments_iPT/iPT_iPT ME 4_DEG.csv")

iPT1

populations_to_test = [iPT1,iPT2,iPT3,iPT4]
...
iPT1.shape
iPT1 = iPT1.drop_duplicates(subset=['Gene'])
...
iPT4=pd.read_csv('/content/iPT4.csv')

iPT4=iPT4.set_index('Unnamed: 0')

gene_rank

gene_rank = iPT1[['names','pi_score']]
...
ax.set_xlabel('')
plt.savefig('iPT4_ME_GSEA.png', dpi=450, bbox_inches='tight')
...
pops=['iPT', 'Fibroblast']
mask=adata.obs['annotation_post_scanvi70_broad'].isin(pops)
subset_iPT_Fibro=adata[mask]
subset_iPT_Fibro

subset_iPT_Fibro.obs['sample'].value_counts()

print(subset_iPT_Fibro[subset_iPT_Fibro.obs['sample']=='HK2844'].obs['CenterY_global_px'])

all_min_distances = []

# Loop through each sample
for sample in subset_iPT_Fibro.obs['sample'].unique():
    subset = subset_iPT_Fibro[subset_iPT_Fibro.obs['sample'] == sample]
    print(sample)
    outliers = subset[subset.obs['iPT_subcluster_ME'] == 'outlier']
    ipt_me4 = subset[subset.obs['iPT_subcluster_ME'] == 'iPT ME 4']
    ...
    distances_df = pd.DataFrame(distances, index=outliers.obs_names)
    min_distances = distances_df.min(axis=1)
    min_distances_df = min_distances.to_frame(name=f'Min_Distance_iPT_ME_4')
...
adata.obs['Min_Distance_iPT_ME_4'] = adata.obs_names.map(result_df['Min_Distance_iPT_ME_4'])
subset_iPT_Fibro.obs['Min_Distance_iPT_ME_4'] = subset_iPT_Fibro.obs_names.map(result_df['Min_Distance_iPT_ME_4'])
# Verify the mapping by printing a sample of the new column
print(adata.obs[['Min_Distance_iPT_ME_4']].head())
...
fibro=subset_iPT_Fibro[subset_iPT_Fibro.obs['annotation_post_scanvi70_broad']=='Fibroblast']
...
df['distance']=fibro.obs['Min_Distance_iPT_ME_4']
...
fibro.obs['Distance_Category'] = pd.cut(fibro.obs['Min_Distance_iPT_ME_4'],
